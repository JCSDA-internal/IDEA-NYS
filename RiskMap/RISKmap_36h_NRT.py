#!/home/hl682259/.conda/envs/luopy/bin/python
...
# add log in folder
# remove date_factor and high resolution IDEA runs
# chang time frame


import os
# os.environ["PROJ_LIB"]='C:\\Users\\Huiying\\Anaconda3\\envs\\luopy\\Library\\share\\basemap'
os.environ['PROJ_LIB']='/home/hl682259/.conda/share/proj'
...
f = open(path_output+'/'+today+'/log', "w")
...
f.write('SNPP number: '+str(len(grid_fileS))+'\n')
f.write('JPSS number: '+str(len(grid_fileJ))+'\n')
...
if ((len(grid_fileS) + len(grid_fileJ) )== 0):#(len(np.argwhere(grid_fileS == 'no-data')) != 0) or (len(np.argwhere(grid_fileJ == 'no-data')) != 0) or (len(np.argwhere(grid_fileH == 'no-data')) != 0)
    ...
    # print('')
    f.write('== NO AVAILABLE IDEA-NYS OUTPUTS for '+today+'\n')
    f.close()
    ...
    f.write('== '+ today +' '+str(tt0) + '-' + str(tt1) + 'H RISK FORECAST'+'\n')
    f.write('== INITIAL:'+ initial_time+ ', FORECAST:'+ forecast_time+'\n')
    ...
    '''Writing NetCDF'''
    output = Dataset(path_output +  '/'+today+'/IDEAlow_36h_risk_map_' + initial_time.replace(' ', '')[
    ...
    plt.savefig(path_output + '/'+today+'/IDEAlow_36h_risk_map_' + initial_time.replace(' ', '')[
                                                                        :-2] + '_' + forecast_time.replace(' ', '')[
                                                                                     :-2] + '.png')
    plt.close()
